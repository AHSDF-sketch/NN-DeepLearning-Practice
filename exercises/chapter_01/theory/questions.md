...
<details style="margin: 10px 0; padding: 8px; background: #f5f5f5; border-radius: 4px;">
  ...
  <div style="margin-top: 8px; padding-top: 8px; border-top: 1px dashed #ccc;">
    <strong>答案</strong>：神经网络是深度学习的基础，深度学习则是神经网络的进一步发展。深度学习通过构建多层神经网络（通常称为“深度神经网络”），实现了对数据的自动特征提取和复杂模式识别，是当前人工智能领域的核心技术之一。<br><br>
    <strong>解析</strong>：传统神经网络层数较少（通常2-3层），难以处理高维复杂数据；而深度学习通过增加网络深度（如10层以上），结合卷积、循环等特殊结构，解决了传统神经网络的局限，在图像、语音、文本等领域取得突破。
  </div>
</details>
...
<details style="margin: 10px 0; padding: 8px; background: #f5f5f5; border-radius: 4px;">
  <summary style="cursor: pointer; font-weight: bold;">点击查看答案</summary>
  <div style="margin-top: 8px; padding-top: 8px; border-top: 1px dashed #ccc;">
    <strong>答案</strong>：CNN在图像识别中的优势主要有三点：一是通过卷积操作提取图像局部特征（如边缘、纹理），符合图像的空间结构特性；二是通过权值共享大幅减少参数数量，降低计算成本和过拟合风险；三是通过池化层（如最大池化）压缩特征维度，增强模型对图像平移、缩放的鲁棒性。<br><br>
    <strong>解析</strong>：传统全连接神经网络将图像拉平为一维向量，丢失空间信息且参数过多；而CNN保留图像二维结构，通过局部感知和权值共享，在保证识别精度的同时提升效率。
  </div>
</details>
...
<details style="margin: 10px 0; padding: 8px; background: #f5f5f5; border-radius: 4px;">
  <summary style="cursor: pointer; font-weight: bold;">点击查看答案</summary>
  <div style="margin-top: 8px; padding-top: 8px; border-top: 1px dashed #ccc;">
    <strong>答案</strong>：激活函数是神经网络中定义在神经元上的非线性函数，用于将神经元的线性输出（加权和+偏置）转换为非线性输出。其核心作用是为神经网络引入非线性，使模型能够拟合复杂的非线性数据分布和函数关系。<br><br>
    <strong>解析</strong>：若没有激活函数，无论神经网络有多少层，整体仍为线性模型（线性变换的叠加仍是线性），无法处理异或、图像分类等非线性问题；常见的激活函数如ReLU、sigmoid等，通过不同的非线性映射满足不同场景需求。
  </div>
</details>
...
<details style="margin: 10px 0; padding: 8px; background: #f5f5f5; border-radius: 4px;">
  <summary style="cursor: pointer; font-weight: bold;">点击查看答案</summary>
  <div style="margin-top: 8px; padding-top: 8px; border-top: 1px dashed #ccc;">
    <strong>答案</strong>：反向传播算法是训练多层神经网络的核心算法，原理可分为两步：一是“前向传播”，将输入数据传入网络，计算各层输出直至得到预测值，并计算预测值与真实值的损失；二是“反向传播”，从输出层开始，利用链式求导法则计算损失函数相对于各层参数（权重、偏置）的梯度，再根据梯度通过梯度下降法更新参数，最终最小化损失。<br><br>
    <strong>解析</strong>：反向传播的关键是“梯度的反向传递”——输出层的梯度直接由损失函数计算，隐藏层的梯度则依赖于后一层的梯度，通过这种方式将误差从输出层传递到输入层，指导所有参数的优化。
  </div>
</details>
...
<details style="margin: 10px 0; padding: 8px; background: #f5f5f5; border-radius: 4px;">
  <summary style="cursor: pointer; font-weight: bold;">点击查看答案</summary>
  <div style="margin-top: 8px; padding-top: 8px; border-top: 1px dashed #ccc;">
    <strong>答案</strong>：梯度下降法是一种常用的优化算法，基本思想是：为了找到损失函数的最小值，先计算损失函数在当前参数处的梯度（梯度方向是函数值上升最快的方向），再沿着梯度的反方向（函数值下降最快的方向）逐步更新参数，重复这一过程直至损失函数收敛到最小值（或接近最小值）。<br><br>
    <strong>解析</strong>：梯度下降法的核心是“沿梯度反方向更新”，通过设置学习率控制每次参数更新的步长（步长过大会震荡不收敛，过小则训练缓慢）；常见的变种如随机梯度下降（SGD）、Adam等，通过改进更新策略提升训练效率。
  </details>
...
function checkMulti(questionName, correctIds, explanation) {
  ...
  if (isCorrect) {
    ...
    resDiv.innerHTML = `<span style='color: #d62728;'>❌ 回答错误！</span><br>正确答案：${correctLabels}<br>${explanation}`;
  }
}

// 填空题判分函数（忽略空格和大小写，兼容多种输入格式）
